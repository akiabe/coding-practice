...
load('result-model8-1.RData')
load('result-model8-2.RData')
load('result-model8-3.RData')
...
dx <- data.frame(t(sapply(
  split(d, d$KID), 
  function(x) range(x$X)
)))
